...
def get_ai_completion(prompt: str, system_prompt: str = None, ai_config: dict = None):
    """
    ...
    ai_config: optional dict with Azure OpenAI credentials:
        {
            ...
            "api_key": "...",
            "chat_model": "gpt-5.2-chat",           # deployment name
...
import logging
from services.vector_store import search_vector_store, search_across_documents
import requests
...
# Ollama client (via get_ai_completion helper above)
client = None  # Not used — all calls go through get_ai_completion()

CHAT_MODEL = settings.OLLAMA_CHAT_MODEL  # llama3.1:8b via Ollama
...
def synthesize_web_and_document_results(query, document_answer, web_results):
    ...
    try:
        ...
        response_text = get_ai_completion(synthesis_prompt, system_prompt="You are LedgerLM, an expert financial analysis assistant that synthesizes document analysis with current web information to provide comprehensive institutional-grade analysis.")
...
def detect_multi_entity_query(query: str) -> dict:
    """
    Detect if query involves multiple geographic entities (countries/regions).
    Returns info about entities mentioned and recommended retrieval strategy.
    """
    query_lower = query.lower()
    
    # Known entities in KPI data
    known_entities = {
        "india": "India",
        "vietnam": "Vietnam", 
        "mexico": "Mexico",
    ...
    # Comparison keywords
    comparison_keywords = ["compare", "vs", "versus", "comparison", "between", "and"]
    ...
    # Determine if this is a multi-entity comparison
    is_multi_entity = len(mentioned_entities) >= 2
    has_comparison_keyword = any(kw in query_lower for kw in comparison_keywords)
    
    # Calculate recommended top_k based on entity count
    # Ensure at least 5 chunks per entity for comprehensive comparison
    if is_multi_entity:
        recommended_top_k = max(15, len(mentioned_entities) * 5)
    ...
    return {
        ...
        "has_comparison": has_comparison_keyword,
        "entities": mentioned_entities,
        "recommended_top_k": recommended_top_k,
    }


def ensure_balanced_entity_coverage(chunks: list, entities: list, min_per_entity: int = 3) -> list:
    """
    Ensure chunks include balanced coverage across all requested entities.
    Prevents one entity from dominating similarity results in comparison queries.
    """
    if not chunks or not entities:
        return chunks
    
    # Group chunks by entity
    entity_chunks = {entity: [] for entity in entities}
    other_chunks = []
    
    for chunk in chunks:
        chunk_text = chunk.get("text", "").lower()
        chunk_entity = chunk.get("metadata", {}).get("entity", "") if isinstance(chunk.get("metadata"), dict) else ""
        
        matched = False
        for entity in entities:
            entity_lower = entity.lower()
            # Check both chunk text content and metadata entity field
            if entity_lower in chunk_text or entity_lower in chunk_entity.lower():
                entity_chunks[entity].append(chunk)
                matched = True
                break
    ...
    # Build balanced result: take min_per_entity from each, then fill with remaining
    balanced_result = []
    remaining_chunks = []
    
    for entity in entities:
        entity_list = entity_chunks[entity]
        balanced_result.extend(entity_list[:min_per_entity])
        remaining_chunks.extend(entity_list[min_per_entity:])
    
    # Add remaining chunks up to reasonable limit
    remaining_chunks.extend(other_chunks)
    balanced_result.extend(remaining_chunks[:10])
    
    # Log coverage stats
    coverage_stats = {entity: len(entity_chunks[entity]) for entity in entities}
    logger.info(f"Entity coverage before balancing: {coverage_stats}")
...
def get_answer(query,
    ...
    """
    try:
        # Detect multi-entity queries and adjust retrieval strategy
        entity_detection = detect_multi_entity_query(query)
        if entity_detection["is_multi_entity"] or entity_detection["has_comparison"]:
            top_k = max(top_k, entity_detection["recommended_top_k"])
            logger.info(f"Multi-entity query detected: {entity_detection['entities']}, using top_k={top_k}")
        ...
        if is_comparison and document_results:
            ...
            # Ensure balanced entity coverage for multi-entity comparison queries
            if entity_detection["is_multi_entity"] and entity_detection["entities"]:
                balanced_chunks = ensure_balanced_entity_coverage(
                    chunks_with_metadata, 
                    entity_detection["entities"],
                    min_per_entity=3
                )
                if balanced_chunks:
                    chunks_with_metadata = balanced_chunks
                    logger.info(f"Applied balanced entity coverage: {len(chunks_with_metadata)} chunks across {entity_detection['entities']}")
        ...
        - ONLY use information that is EXPLICITLY in the provided document context
        - NEVER fabricate, estimate, or make up values not found in the documents
        - If the information is not available, clearly say "This information is not available in the provided documents"
...
- ONLY provide information that is EXPLICITLY stated in the provided document context
- NEVER make up, fabricate, or estimate values that are not in the documents
- If data is not available in the context, clearly state "This information is not available in the provided documents"
- Do NOT speculate about future values or trends unless asked for forecasts based on available data
- When citing numbers, only use values that appear verbatim in the context
- If asked about data not in the documents, say "I don't have this information in the current documents"
- NEVER invent company names, dates, or financial figures not in the source material"""
...
6. Highlight any significant deviations or patterns in the data"""
        ...
        # Generate the answer using Ollama via get_ai_completion
        logger.info(f"Sending query to Ollama: {query}")
        try:
            answer = get_ai_completion(user_prompt, system_prompt)
            ...
            if table_formatting['has_structured_data']:
                ...
                table_section += "Below are the detailed financial tables from your company data with analytical insights:\n\n"
...
Focus on delivering substantial analysis with real calculations based on the actual financial data provided. Use the specific numbers, ratios, and metrics from the data sources to provide actionable investment insights."""
        ...
        analysis = get_ai_completion(user_prompt, system_prompt)
