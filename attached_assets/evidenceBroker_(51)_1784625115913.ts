...
export class EvidenceBroker {
  ...
  private buildPreBuiltSections(
    ...
    // This flag is ONLY true for investment_capex_pmo; all KPI/cost paths are unaffected.
    const isInvestment = primary.calculationType === 'investment_capex_pmo';
    const plainNum = (n: number): string =>
      ...
      for (const valCol of valueColumns) {
        ...
        nonTotalPivotEntries.forEach(([, { dims, byPeriod }]) => {
          ...
          const monthVals = sortedPeriods.map(({ m, y }) => {
            ...
            return isInvestment ? plainNum(v) : this.formatValue(v, valCol, currency, userQuery, !rawUSD);
          });
        ...
        if (!isPercentCol && !isSingleEntityPivot) {
          const totalMonthVals = sortedPeriods.map(({ m, y }) => {
            const tv = colPeriodTotals[`${y}_${String(m).padStart(2, "0")}`];
            return isInvestment ? plainNum(tv) : this.formatValue(tv, valCol, currency, userQuery, !rawUSD);
          });
          const totalYtd = ytdLabel
            ? (() => {
                const tv = sortedPeriods.reduce(
      ...
      for (const valCol of valueColumns) {
        ...
        nonTotalXEntries.forEach(([key, dims]) => {
          ...
          const periodFmt = periodValsRaw.map((v) =>
            isInvestment ? plainNum(v) : this.formatValue(v, valCol, currency, userQuery, !rawUSD),
          );
          const ytdSum = periodValsRaw.reduce((s, v) => s + v, 0);
          const ytdFmt = xYtdLabel
            ? (isInvestment ? plainNum(ytdSum) : this.formatValue(ytdSum, valCol, currency, userQuery, !rawUSD))
            : null;
          const share: string[] = [];
          table +=
            "| " +
        ...
        if (!xIsPercentCol && !xIsSingleEntity) {
          const totalPeriodFmt = allEvidences.map((_, pi) =>
            isInvestment
              ? plainNum(colPeriodTotals[pi])
              : this.formatValue(colPeriodTotals[pi], valCol, currency, userQuery, !rawUSD),
          );
          const totalYtd = xYtdLabel
            ? (() => {
                const tv = Object.values(colPeriodTotals).reduce((s, v) => s + v, 0);
                return isInvestment ? plainNum(tv) : this.formatValue(tv, valCol, currency, userQuery, !rawUSD);
              })()
            : null;
          table +=
    ...
    rows.forEach((row) => {
      ...
      const cells = displayCols.map((col) => {
        ...
        if (!isNaN(num))
          return isInvestment
            ? plainNum(num)
            : this.formatValue(num, col, currency, userQuery, !rawUSD);
        return String(v);
      });
      table += "| " + cells.join(" | ") + " |\n";
    });
